...
namespace Tests\PHPUnit\Modules\Kanban\Admin;
...
class AdminTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testRequestLoads()
    {
        ...
        $request = new Request(new Http('http://127.0.0.1/en/backend/kanban'));
        ...
        foreach ($loaded[4] as $module) {
            if ($module['module_load_file'] === 'Kanban') {
                $found = true;
                break;
            }
        }
        ...
        self::assertTrue(in_array('Kanban', $moduleManager->getRoutedModules($request)));

        $moduleManager->initRequestModules($request);
        self::assertTrue(isset(TestUtils::getMember($moduleManager, 'running')['Kanban']));
    }
}
