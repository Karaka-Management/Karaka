(() => {
  // Install/Application/Backend/js/sw.js
  var PRECACHE_VERSION = "static-v1.0.0";
  var RUNTIME_VERSION = "runtime-v1.0.0";
  var PRECACHE_URLS = [
    "cssOMS/styles.css",
  ...
  self.addEventListener("install", (event) => {
    event.waitUntil(
      caches.open(PRECACHE_VERSION).then((cache) => cache.addAll(PRECACHE_URLS)).then(self.skipWaiting())
    );
    setInterval(fetchNotification, 5 * 60 * 1e3);
  });
  var offlineMessageHTML = '<div style="position: fixed; bottom: 0; left: 0; width: 100%; background-color: red; color: white; text-align: center; padding: 10px;">Offline Mode</div>';
  ...
  function fetchNotification() {
    ...
    fetch("/api/notification?start=" + encodeURIComponent(dt.getFullYear() + "-" + (dt.getMonth() + 1) + "-" + dt.getDate() + " " + dt.getHours() + ":" + dt.getMinutes() + ":" + dt.getSeconds())).then((response) => {
      ...
      if (typeof data[0] !== "undefined" && data[0].response.length > 0) {
