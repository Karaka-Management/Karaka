(() => {
  ...
  var UriFactory = class _UriFactory {
    ...
    static build(uri, toMatch = null) {
      const current = typeof window === "undefined" ? "" : HttpUri.parseUrl(window.location.href);
      ...
      let parsed = uri.replace(new RegExp("{[/#?%@.$!].*?}", "g"), function(match) {
        ...
        if (toMatch !== null && (Object.prototype.hasOwnProperty.call(toMatch, match) || match.includes("/"))) {
          return match.includes("/") ? jsOMS2.getArray(match, toMatch) : toMatch[match];
      ...
      request.setResultCallback(0, function(xhr) {
        ...
        if (xhr.getResponseHeader("content-type").includes("application/octet-stream")) {
          ...
          try {
            ...
            } else if (typeof responseData[0].status !== "undefined" && responseData[0].status !== NotificationLevel.HIDDEN) {
              self2.app.notifyManager.send(
                new NotificationMessage(
        ...
        if (redirect !== null && (statusCode === 200 || statusCode === null)) {
          ...
          fetch(redirectUrl).then((response) => response.text()).then((html) => {
